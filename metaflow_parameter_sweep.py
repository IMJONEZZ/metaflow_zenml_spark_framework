...
class MetaflowParameterSweepFlow(FlowSpec):
    ...
    learning_rates = Parameter('learning_rates',
    ...
    max_parallel_runs = Parameter('max_parallel_runs',
    ...
    def start(self):
        ...
        print("🚀 Starting Advanced Metaflow Parameter Sweep")
        ...
        try:
            self.lr_list = [float(lr.strip()) for lr in str(self.learning_rates).split(',')]
            ...
            print(f"Warning: Could not parse parameters. Using defaults.")
        ...
        import itertools
        self.parameter_combinations = list(itertools.product(
            self.lr_list, 
            self.bs_list, 
            self.model_list
        ))
        ...
        if len(self.parameter_combinations) > max_runs:
            print(f"Limiting to {max_runs} parameter combinations for demo")
            self.parameter_combinations = self.parameter_combinations[:max_runs]
        
        print(f"Total parameter combinations to test: {len(self.parameter_combinations)}")
    ...
    def generate_data(self):
        ...
        print(f"Data split: {len(self.X_train)} train, {len(self.X_val)} validation samples")
    ...
    def train_models_parallel(self):
        ...
        print(f"🏋️ Training {len(self.parameter_combinations)} models with parameter sweeps...")
    ...
    def end(self):
        ...
        print(f"Total Models Trained: {self.performance_summary['total_models_tested']}")
        ...
        if 'performance_ranking' in self.performance_summary:
            ...
            for i, model in enumerate(self.performance_summary['performance_ranking'][:3]):
                print(f"  {i+1}. {model['run_id']}: {model['val_accuracy']:.4f} validation accuracy")

    def _train_single_model(self, run_id: str, learning_rate: float, 
                           batch_size: int, model_type: str) -> dict:
        ...
        try:
            ...
            model = SimpleModel(input_size=20).to(device) if model_type == 'simple' else DeepModel(input_size=20).to(device)
            ...
            for epoch in range(self.epochs_value):
                ...
                # Validation
                if epoch % 2 == 0 or epoch == self.epochs_value - 1:  # Check every 2 epochs
                    model.eval()
            ...
            # Final accuracy
            final_train_acc = self._evaluate_model(model, X_train_tensor, y_train_tensor)
