...
def generate_time_series(num_points: int = 200) -> List[float]:
    ...
    temperature = 20 + 0.1 * time + 5 * np.sin(2 * np.pi * time / 50) + np.random.normal(0, 1, num_points)
...
@pipeline
def simple_timeseries_pipeline(num_points: int = 200, sequence_length: int = 10) -> Dict:
